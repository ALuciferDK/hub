        ...
        if($request->isMethod('post'))//判断是否是post请求，是post请求开始进行登录判断
        {
            ...
            if($result)
            {
                ...
                if($pwd == $result['u_pwd'])
                {
                    ...
                    return redirect('/message')->with(['message'=>'登录成功!','url' =>'/first/first', 'jumpTime'=>3,'status'=>true]);
                }
                else
                {
                    return redirect('/message')->with(['message'=>'密码错误!','url' =>'/user/login', 'jumpTime'=>3,'status'=>false]);
                }

            }else
            {
                return redirect('/message')->with(['message'=>'账号不存在!','url' =>'/user/login', 'jumpTime'=>3,'status'=>false]);
            }
        }
